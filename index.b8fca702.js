...
function handleTouchEnd(touchEvent) {
    ...
    if (Math.abs(deltaX) > 10 || Math.abs(deltaY) > 10) {
        ...
        } else if (deltaY > 0) game.moveDown();
        else game.moveUp();
...
function handleMoveOutcome(numbersMove) {
    if (numbersMove) {
        ...
        if (newStatus === Game.Status.win) winMessage.classList.remove("hidden");
        else if (newStatus === Game.Status.lose) loseMessage.classList.remove("hidden");
    }
}
